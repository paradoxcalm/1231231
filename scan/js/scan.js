...
function logQrEvent(event, details = {}) {
    try {
        fetch('scan/api/log_qr_scan.php', {
            method: 'POST',
...
async function startPhotoCamera() {
    ...
    try {
        ...
        console.warn('Не удалось открыть камеру для фото', err);
        const fallback = document.getElementById('receptionPhoto');
        const video = document.getElementById('photoVideo');
...
function takePhoto() {
    ...
    canvas.toBlob((blob) => {
        if (blob) {
            ...
            document.getElementById('confirmReceptionBtn').disabled = false;
            const btn = document.getElementById('takePhotoBtn');
            if (btn) btn.style.display = 'none';
            if (video) video.style.display = 'none';
            if (photoStream) {
...
async function showReceptionModal(data, qrText) {
    ...
    overlay.innerHTML = `
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-section">
                    ...
                    <div id="photoArea">
                        ...
                        <input type="file" id="receptionPhoto" accept="image/*" capture="environment" style="display:none;" />
                    </div>
                </div>
            </div>
            <div class="modal-actions">
                ...
                <button id="confirmReceptionBtn" class="confirm-btn" onclick="submitReception(${data.order_id}, '${qrText}', 'confirm')" disabled>✅ Подтвердить</button>
    ...
    document.getElementById('receptionPhoto').addEventListener('change', (e) => {
        if (e.target.files[0]) {
...
async function submitReception(orderId, qrCode, action) {
    ...
    // Добавляем фото
    if (photoBlob) {
        formData.append('photo', photoBlob, `order_${orderId}.jpg`);
    } else {
    ...
    try {
        const response = await fetch('scan/api/confirm_reception.php', {
            method: 'POST',
            body: formData
        });
        if (!response.ok) {
