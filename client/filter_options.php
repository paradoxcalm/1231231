...
switch ($action) {
    ...
    case 'all_cities':
        $res = $conn->query("SELECT DISTINCT city FROM schedules WHERE city IS NOT NULL AND city != ''");
        $cities = [];
        while ($row = $res->fetch_assoc()) $cities[] = $row['city'];
        echo json_encode(['success' => true, 'cities' => $cities]);
        break;

    // --- ВСЕ СКЛАДЫ (по всем городам/маркетплейсам или по конкретному городу) ---
    case 'all_warehouses':
        $sql = "SELECT DISTINCT warehouses FROM schedules WHERE warehouses IS NOT NULL AND warehouses != ''";
        $params = [];
        $types = "";
        if (!empty($_GET['city'])) {
            $sql .= " AND city = ?";
        ...
        $res = $stmt->get_result();
        $whs = [];
        while ($row = $res->fetch_assoc()) $whs[] = $row['warehouses'];
        $stmt->close();
